...
FORBIDDEN_TAGS = [ "phone", "website", "contact:phone", "contact:website", "contact:email", "contact:facebook" ]

def display_name(item: Element):
    return item.attrib["ka.name"] if "ka.name" in item.attrib else item.attrib["name"]
...
def check_wikidata_tag(wd: dict, item, key: str, lang: str) -> Generator[Issue]:
    for k in filter(lambda x: x.attrib["key"] == key, item.findall("josm:key", ns)):
        if "labels" in wd and lang in wd["labels"]:
            yield from issues_with_tag(k, match_values(f"wikidata label for language {lang}", wd["labels"][lang], f"key {key}", k.attrib["value"]))
...
def check_wikidata_family(item: Element, tag_family: str):
    ...
    if qid:
        ...
        yield from check_wikidata_tag(wd, item, tag_family, DEFAULT_LANG)
        for lang in LANGS:
...
def check_forbidden_tags(item):
    for key in item.findall("josm:key", ns):
        if key.attrib["key"] in FORBIDDEN_TAGS:
            yield Issue(f"do not use tag {key.attrib["key"]}; upload to wikidata instead", tag=key)
...
def check_group(group: Element, diff_map: dict):
    for item in group.findall('josm:item', ns):
        ...
        if name not in diff_map or item_tags(item) != diff_map[name]:
            for check in [check_forbidden_tags, check_wikidata]:
...
def __main__():
    ...
    if root_group is not None:
        for issue in check_group(root_group, diff_map):
            print(f"{input_file}#{issue.location()}: {logging.getLevelName(issue.severity)}: {display_name(issue.item)}: {issue.message}")
            exit_code = 1
